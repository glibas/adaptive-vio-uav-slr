...
Data, code, and decision trail for the systematic literature review *Adaptive Visual-Inertial Odometry Methods for UAV Navigation in GNSS-Denied Environments*, from database export to synthesis. Every number, table, and figure in the paper traces back to its source records and can be recomputed.
...
| `06_data_extraction/` | Data extraction | 20-field extraction table for the 86 included studies |
...
**Retrieval.** Full texts of the 232 retained records were sought through institutional access, open-access repositories, publisher platforms and author requests. 124 were obtained; 108 could not be retrieved through any channel and were excluded (EXC-8). `03_retrieval/retrieval_status.csv` records the outcome per report.

**Eligibility.** The 124 retrieved reports were read in full and assessed against three hard filters: T-1, the paper makes an algorithmic contribution to VIO rather than consuming it; T-2, the method is validated on aerial data; T-3, a quantitative evaluation is reported. Six binary relevance dimensions (GNSS-denied operation, adaptive component, real-time embedded operation, filter-based fusion, feature front-end, auxiliary sensor) were scored alongside. 38 reports were excluded: 32 failed T-1 (12 of which also failed T-3), 3 under the language criterion (INC-4), and 3 under EXC-9 (research of russian origin), giving a final synthesis corpus of **86 studies** (2015–2026).

**Quality assessment.** Each retained paper was scored on the five items of Dybå and Dingsøyr (clear objective; experimental setup described; baseline comparison; limitations discussed; conclusions supported), each at 0 / 0.5 / 1.0. Mean total 4.30, minimum 2.5, maximum 5.0; no paper fell below the 2.0 sensitivity threshold.
...
**PRISMA flow.** 648 identified → 250 duplicates removed → 398 screened → 166 excluded → 232 sought → 108 not retrieved → 124 assessed → 38 excluded → 86 included. The rendered diagram is in `07_synthesis/prisma/`.
