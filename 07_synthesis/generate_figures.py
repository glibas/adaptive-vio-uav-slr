...
def fig_gap_matrix():
    ...
    matrix = [
        ('Visual-quality\nweighting',
         [('Frame-level', 'full'), ('Motion-state', 'part'),
          ('Per-feature /\nkeypoint\nG1', 'gap')]),
        ('Height-channel\nfusion',
         [('Fixed-weight', 'full'), ('\u2014', 'none'),
          ('Adaptive\nquality-wtd.\nG3', 'gap')]),
        ('Mode switching /\nfault detection',
         [('Single\ntrigger', 'full'), ('\u2014', 'none'),
          ('Multi-criterion\n(cam+IMU+baro)\nG4', 'gap')]),
    ...
    leg = [mpatches.Patch(color=FULL, label='Established in corpus'),
           mpatches.Patch(color=PART, label='Partially addressed'),
           mpatches.Patch(color=NONE_C, label='Not addressed'),
